...
module.exports = function (hoodie, pluginDb) {
  ...
  var replicatorDoc = function (subject, sourceDbName, targetDbName) {
    ...
    return {
      ...
      source: sourceDbName,
      target: targetDbName,
      filter: 'filters/profile_by_type',
      query_params: {
        type: subject
      },
      user_ctx: {
  ...
  var _getByUsername = function (task, cb) {
    ...
    pluginDb.query('by_userName', { include_docs: true, key: task.get.userName }, function (err, rows) {
  ...
  Profile.getByUserName = function (db, task) {
    ...
    async.series([
      ...
      utils.handleTask(hoodie, 'getByUserName', db, task)
    );
  };
