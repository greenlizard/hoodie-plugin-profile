...
module.exports = function (hoodie, pluginDb) {
  ...
  var _isSubscribed = function (task, cb) {
    ...
    replicator.find(task.profile.subject, subscribeId, function (err, _doc) {
      task.profile.isSubscribed = (err && err.error === 'not_found') ? false : !!_doc;
      cb(null, task);
    });
  };
  ...
  var _search = function (task, cb) {
    log('_search', task);
    pluginDb.queryFti('by_fti', { include_docs: true, q: task.profile.term  }, function (err, rows) {
      if (err || !rows.length) return cb(err || new Error('not_found'));
      task.profile.search = rows.map(function (v) {
        return v.doc;
      });
      cb(null, task);
    });
  ...
  Profile.search = function (db, task, cb) {
    log('search', task);

    async.series([
        async.apply(_validAttrs, task, 'profile'),
        async.apply(_validAttrs, task.profile, 'term'),
        async.apply(_search, task)
      ],
      utils.handleTask(hoodie, 'search', db, task, cb)
    );
  };
